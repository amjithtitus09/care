...
def get_patient_bucket_config(external) -> tuple[ClientConfig, BucketName]:
    if settings.CS_PROVIDER == CSProvider.AWS.value:
        ...
        return {
            "connection_string": settings.PATIENT_AZURE_CONNECTION_STRING,
        }, settings.PATIENT_AZURE_CONTAINER
    elif settings.CS_PROVIDER == CSProvider.LOCAL.value:
        return {}, settings.PATIENT_LOCAL_DIR
    raise ValueError("Unsupported Cloud Service Provider")


def get_client_config(
    bucket_type: BucketType, external=False
) -> tuple[ClientConfig, BucketName]:
